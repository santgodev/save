...
async function migrate() {
  ...
  const urlMatch = envText.match(/EXPO_PUBLIC_SUPABASE_URL=([^\n\r]+)/);
  const keyMatch = envText.match(/EXPO_PUBLIC_SUPABASE_ANON_KEY=([^\n\r]+)/);
...
$$;
  ...
  console.log('Sending SQL to Supabase execute_sql RPC...');
  const { data, error } = await supabase.rpc('execute_sql', { query: migrationSql });
  if (error) {
    console.error('Migration failed:', error.message || error);
    console.error('If execute_sql RPC is missing, the user MUST run this SQL manually in the Supabase Dashboard.');
    fs.writeFileSync('scratch/manual_migration.sql', migrationSql);
  } else {
