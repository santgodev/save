...
- `register_income(uuid, numeric, jsonb, text, text, text, text)` — distribuye entre bolsillos, `p_cycle_mode: 'accumulate'|'start_fresh'`. Si `start_fresh`, delega en `close_and_start_new_cycle(NULL, p_rollover_mode)`.
- `execute_cycle_closure(uuid, uuid, jsonb)` — cierra un ciclo (`user_closed=true`), barrido opcional de saldos a bolsillo libre (`p_sweeps`). Idempotente desde el fix del 2026-07-26: si el ciclo ya estaba `user_closed`, no vuelve a barrer nada. Hoy solo se dispara desde `MonthClosureModal`, que a su vez solo aparece si algún ciclo queda con `end_date` puesto y `user_closed=false` — algo que `close_and_start_new_cycle` ya no deja pasar (ver abajo), así que este camino está vivo pero dormido: se reactivaría si en el futuro algo más (ej. un cron de cierre automático de mes) deja un ciclo a medio cerrar.
- `close_and_start_new_cycle(text?, text?)` — cierra el ciclo activo (reparte el sobrante según `p_rollover_mode: 'carry_over'|'sweep_to_savings'`) y crea uno nuevo.
  **Bug real, 2026-07-26 (dos capas):**
  1. Primer intento (`20260726000001_fix_cycle_double_closure.sql`): se detectó que el ciclo viejo se quedaba con `end_date` puesto pero `user_closed=false` para siempre, lo que hacía que el Dashboard volviera a mostrar el modal de cierre (`MonthClosureModal`) para un ciclo cuyo sobrante ya se había repartido, y si el usuario seguía ese modal, `execute_cycle_closure` volvía a mover la misma plata (los bolsillos no son por ciclo, son la misma fila para todos los meses). El fix agregó `user_closed = true` a la misma línea que cierra `end_date`.
  2. Ese primer fix no alcanzó — el bloque completo donde vivía (reparto de sobrante + cierre) **nunca se ejecutaba**, por una causa más profunda encontrada después (`20260726000002_fix_record_is_not_null_trap.sql`): la condición de entrada era `IF v_current_cycle IS NOT NULL THEN`, y en Postgres una variable `record` con una mezcla de columnas NULL y no-NULL hace que **tanto** `IS NULL` **como** `IS NOT NULL` evalúen a `FALSO` (comportamiento documentado del tipo fila compuesto). Como `v_current_cycle` se llena con `... WHERE end_date IS NULL`, cualquier fila que encuentre siempre tiene `end_date = NULL` — un registro "mixto" por construcción — así que la condición nunca era verdadera, sin importar qué ciclo hubiera. Fix real: usar la variable especial `FOUND` de PL/pgSQL en vez de comparar el record contra NULL.
  Diagnosticado en vivo contra la base de datos (no reproducible solo leyendo el código) — si vuelves a ver "record IS NOT NULL" en otra función después de un SELECT filtrado por una columna nullable, sospecha de esto primero.
  **Seguimiento (`20260726000003`/`20260726000004`):** una barrida completa del schema encontró el mismo patrón en `get_cycle_state` (dos veces — una en el cálculo de `v_prev_income`/`v_prev_spent`, otra 30 líneas más abajo al armar el campo `previous_month` del JSON, esta segunda inicialmente pasada por alto) y confirmó que en `execute_cycle_closure` el check equivalente (`v_cycle IS NULL`) nunca estuvo realmente en riesgo — ahí `id`/`user_id` son `NOT NULL`, así que un registro encontrado nunca puede salir "mixto" en esa dirección; se cambió a `FOUND` de todas formas por consistencia, sin que corrigiera un bug real. Lección operativa: `FOUND` se pisa con cada `SELECT INTO`/`UPDATE`/etc. posterior — si necesitas el resultado de un SELECT más adelante en la función, guárdalo en una variable propia, no vuelvas a leer `FOUND`. **Estado: resuelto y confirmado en producción el 2026-07-26** — `get_cycle_state` probado contra un ciclo con mes anterior real, `previous_month` se arma correctamente y `prev_month_closed` refleja el valor real.
