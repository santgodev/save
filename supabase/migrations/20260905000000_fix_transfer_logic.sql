-- Fix bug where Libre pocket did not update its allocated_budget on transfers
...
DECLARE
  ...
  v_from_cat    TEXT;
  v_to_cat      TEXT;
  v_from_is_free BOOLEAN;
  v_to_is_free   BOOLEAN;
BEGIN
  -- 🔒 Seguridad
  ...
  -- Origen
  SELECT name, category, is_default_free
  INTO v_from_name, v_from_cat, v_from_is_free
  ...
  -- Destino
  SELECT name, category, is_default_free
  INTO v_to_name, v_to_cat, v_to_is_free
  ...
  -- 🔁 Movimiento de Presupuesto (Fixed: Libre pocket MUST update its budget)
...
-- Fix bug where deleting a transfer failed to delete its pair because of wrong category
...
BEGIN
  ...
  IF v_type = 'internal_transfer_out' THEN
     DELETE FROM transactions WHERE user_id = p_user_id AND cycle_id = v_cycle_id AND metadata->>'type' = 'internal_transfer_in' AND metadata->>'from_id' = v_metadata->>'from_id' AND metadata->>'to_id' = v_metadata->>'to_id';
  ELSIF v_type = 'internal_transfer_in' THEN
     DELETE FROM transactions WHERE user_id = p_user_id AND cycle_id = v_cycle_id AND metadata->>'type' = 'internal_transfer_out' AND metadata->>'from_id' = v_metadata->>'from_id' AND metadata->>'to_id' = v_metadata->>'to_id';
  END IF;
