...
export const Dashboard = ({ transactions }: { transactions: any[] }) => {
  ...
  useEffect(() => {
    ...
    if (hours < 12) setGreeting('┬íBuenos d├¡as! ÔÿÇ´©Å');
    else if (hours < 18) setGreeting('┬íBuenas tardes! Ôÿò');
    else setGreeting('┬íBuenas noches! ­ƒîÖ');
  ...
  // Nueva detecci├│n reactiva: Se activa cuando las reglas O las transacciones cambian
  useEffect(() => {
    analyzePatterns();
  ...
  const analyzePatterns = () => {
    ...
    // EVITAR PARPADEO: Solo actualizamos si realmente hay un cambio o si no hay patr├│n actual
  ...
  const saveRule = async (type: 'confidence' | 'monitor' | 'reduce') => {
    ...
    // Optimistic update para feedback instant├íneo
...
Habla de forma humana, emp├ítica y cercana (Colombia).
ACT├ÜA SEG├ÜN LAS REGLAS DEL USUARIO:
- confidence: Gastos esenciales. No los critiques, solo conf├¡rmalos.
- monitor: Gastos bajo observaci├│n. Menciona tendencias.
- reduce: Gastos a recortar. Alerta activamente pero sin juzgar.

DIFERENCIACI├ôN DE INSIGHTS:
- Si detectas algo inusual, di: "Se te est├í yendo plata en..."
- Si algo mejora, di: "Vas muy bien con..."

JSON: { 
  mainInsight: string (m├íx 60 car), 
  reason: string (explicaci├│n emp├ítica), 
  ...
  secondaryInsights: string[] 
      ...
      setRecommendation({ 
        mainInsight: "┬íVas por buen camino!", 
        reason: "Analizar├® tus gastos cuando tengas m├ís registros para darte consejos ├║nicos.", 
        type: "necesario" 
      }); 
      ...
      >
        ...
        <BlurView intensity={95} tint="light" style={styles.organicAiCard}>
          <View style={styles.iaHeader}>
            <View style={{ flex: 1 }}>
              <Text style={styles.iaLabel}>AN├üLISIS INTELIGENTE</Text>
            </View>
            <TouchableOpacity 
              activeOpacity={0.7} 
              ...
              {recommendation.secondaryInsights?.length > 0 && (
                <View style={styles.iaTipBox}>
                  <Text style={[styles.iaTipBody, { backgroundColor: theme.colors.primaryContainer, color: theme.colors.onPrimaryContainer }]}>
                    Ô£¿ TIP: {recommendation.secondaryInsights[0]}
                  </Text>
                </View>
              )}
            </View>
        ...
        {detectedPattern && (
          <View style={styles.patternBox}>
             <View style={{flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom:16}}>
                ...
                <View style={{flex: 1}}>
                  <Text style={styles.patternHead}>Nueva Regla ­ƒî┐</Text>
                  <Text style={styles.patternSub}>┬┐C├│mo clasificas tus gastos en {detectedPattern.merchant}?</Text>
                </View>
